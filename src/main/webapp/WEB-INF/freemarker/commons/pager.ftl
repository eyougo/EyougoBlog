<#macro p perpage totalnum offset params='' maxsteps=6>  
    ...
    <#-- 首页 -->  
    <#if ipage gt 1>  
        <a href="?pager.offset=0${requestParams}">&lt;&lt;</a>  
    <#else>  
        <span class="disabled">&lt;&lt;</span>  
    </#if>  
    <#-- 前组-->  
    <#if stepsNow gt 0>  
        <a href="?pager.offset=${stepsNow * maxsteps * iperpage}${requestParams}">…</a>  
    <#else>  
        <span class="disabled">…</span>  
    </#if>  
    <#-- 当前组中的页号-->  
    ...
    <#if ipage gt 0 && ipage lte totalpage>  
        <#list (stepsNow * maxsteps + 1)..pagelist as num>  
            <#if ipage != num>  
                <a href="?pager.offset=${(num-1) * iperpage }${requestParams}">${num}</a>  
            <#else>  
                <strong>${num}</strong>  
            </#if>  
        </#list>  
    ...
    <#if stepsNow lt stepsLast>  
        <a href="?pager.offset=${(stepsNow + 1) * maxsteps * iperpage}${requestParams}">…</a>  
    <#else>  
        <span class="disabled">…</span>  
    </#if>  
    <#-- 尾页 -->  
    <#if ipage lt totalpage>  
        <a href="?pager.offset=${(totalpage - 1)*iperpage}${requestParams}">&gt;&gt;</a>  
    <#else>  
        <span class="disabled">&gt;&gt;</span>  
    </#if>  
    <#-- 前一页 -->  
    <#if ipage gt 1>  
        <a href="?pager.offset=${ioffset - iperpage}${requestParams}"><@s.text name="pager.previous"/></a>  
    <#else>  
        <span class="disabled"><@s.text name="pager.previous"/></span>  
    </#if>  
    <#-- 后一页 -->  
    <#if ipage lt totalpage>  
        <a href="?pager.offset=${ioffset + iperpage}${requestParams}"><@s.text name="pager.next"/></a>  
    <#else>  
